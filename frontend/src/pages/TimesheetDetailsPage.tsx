...
const TimesheetDetailsPage: React.FC = () => {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      alert(err?.response?.data?.error || 'Failed to approve timesheet')
    }
  }
    ...
    return (
      <SidebarLayout>
        <div className="flex flex-col items-center justify-center py-40 space-y-3">
          ...
          <p className="text-zinc-500 text-sm font-medium">Loading timesheet details...</p>
        </div>
      </SidebarLayout>
    )
  }
    ...
    return (
      <SidebarLayout>
        <div className="p-8 text-center text-rose-400 bg-rose-500/10 border border-rose-500/20 rounded-xl space-y-4 max-w-md mx-auto">
          ...
          <p className="text-xs text-zinc-400">The requested timesheet could not be fetched or does not exist.</p>
          <Link to="/timesheets" className="inline-flex text-xs font-bold text-violet-400 hover:text-violet-300">
            Back to Timesheets
          </Link>
        </div>
      </SidebarLayout>
  ...
  const getStatusBanner = () => {
    switch (timesheet.status) {
          ...
          <div className="p-4 bg-rose-500/10 border border-rose-500/20 rounded-xl flex items-start space-x-3 text-rose-400">
            ...
            <div>
              ...
              <p className="text-zinc-400 text-xs mt-1 font-bold">
                Reason: <span className="text-rose-300 font-semibold">{timesheet.rejectionReason}</span>
              </p>
      ...
      case 'SUBMITTED':
        return (
          <div className="p-4 bg-indigo-500/10 border border-indigo-500/20 rounded-xl flex items-start space-x-3 text-indigo-400">
      ...
      default:
        return (
          <div className="p-4 bg-zinc-500/10 border border-zinc-500/20 rounded-xl flex items-start space-x-3 text-zinc-400">
            <Info className="h-5 w-5 shrink-0 mt-0.5" />
            <div>
  ...
  return (
    <SidebarLayout>
      <div className="space-y-6">
        ...
        {/* Header Section */}
        <div className="flex flex-col lg:flex-row justify-between lg:items-center gap-6 border-b border-zinc-800 pb-6">
          <div>
            <div className="flex items-center space-x-3">
              ...
              <span className="text-zinc-500 text-xs font-semibold">• ID: {timesheet.id.substring(0, 8)}</span>
            </div>
            <h1 className="text-3xl font-extrabold text-white mt-2">
              Weekly Timesheet Period
            </h1>
            <p className="text-zinc-350 text-sm mt-1 flex items-center">
              <Calendar className="h-4 w-4 mr-1.5 text-violet-400" />
              {formatDate(timesheet.startDate)} — {formatDate(ts => ts.setDate(ts.getDate() + 6) && timesheet.endDate || timesheet.endDate)}
            </p>
          </div>

          <div className="bg-[#14161f] border border-zinc-800 p-4 rounded-xl flex gap-6">
            <div>
              <span className="block text-[9px] font-black text-zinc-500 uppercase tracking-widest">Project</span>
              <span className="block text-white font-extrabold text-sm mt-0.5">{timesheet.project?.name}</span>
              <span className="block text-zinc-400 text-xs font-semibold">{timesheet.project?.location}</span>
            </div>
            <div className="w-px bg-zinc-800"></div>
            <div>
              <span className="block text-[9px] font-black text-zinc-500 uppercase tracking-widest">Total Hours</span>
              <span className="block text-violet-400 font-black text-lg mt-0.5">{totalHoursCombined} hrs</span>
            </div>
          </div>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-zinc-800/60 text-xs">
                {timesheet.rows?.map((row) => {
                  ...
                  return (
                    <tr key={row.id} className="hover:bg-[#1a1c27]/30 transition-colors">
                      <td className="py-4 px-6 font-bold text-white">
                        {row.worker.name}
                        <span className="block text-[10px] text-zinc-500 font-semibold">{row.worker.trade}</span>
                      </td>
                      <td className="py-4 px-3 text-center font-bold text-zinc-350 bg-zinc-900/10">
                        {row.mondayHours || '-'}
                      </td>
                      <td className="py-4 px-3 text-center font-bold text-zinc-350">
                        {row.tuesdayHours || '-'}
                      </td>
                      <td className="py-4 px-3 text-center font-bold text-zinc-350 bg-zinc-900/10">
                        {row.wednesdayHours || '-'}
                      ...
                      <td className="py-4 px-4 text-center font-black text-violet-400">
                        {row.totalHours} hrs
                      </td>
              ...
              <tfoot>
                <tr className="bg-[#181a24]/30 font-black text-xs border-t border-zinc-800 text-white">
                  <td className="py-4 px-6 uppercase tracking-wider">Grand Total</td>
                  <td colSpan={7}></td>
                  <td className="py-4 px-4 text-center text-violet-400 font-black">{totalHoursCombined} hrs</td>
                  <td></td>
                  <td className="py-4 px-6 text-right text-green-400 font-black">{formatCurrency(totalEstimatedPayroll)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
        ...
        {/* Action Panel */}
        <div className="bg-[#14161f] border border-zinc-800 rounded-xl p-6 shadow-xl flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center space-x-3 text-zinc-400 text-xs">
            <Lock className="h-4 w-4 text-zinc-550 shrink-0" />
            <span>
              {timesheet.status === 'APPROVED' 
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            {(timesheet.status === 'DRAFT' || timesheet.status === 'REJECTED') && (
              <button
                ...
                className="w-full md:w-auto inline-flex items-center justify-center px-5 py-2.5 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-colors shadow-lg shadow-violet-600/10 disabled:opacity-50"
              >
            ...
            {timesheet.status === 'SUBMITTED' && (user?.role === 'ADMIN' || user?.role === 'PROJECT_MANAGER') && (
              <>
                <button
                  ...
                  className="flex-1 md:flex-none inline-flex items-center justify-center px-5 py-2.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/20 rounded-lg text-xs font-bold uppercase tracking-wider transition-colors"
                >
                ...
                <button
                  ...
                  className="flex-1 md:flex-none inline-flex items-center justify-center px-5 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-colors shadow-lg shadow-green-600/10 disabled:opacity-50"
                >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#14161f] border border-zinc-800 rounded-xl max-w-md w-full overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-zinc-800 flex justify-between items-center">
              <h3 className="font-extrabold text-sm text-white uppercase tracking-wider">Reject Weekly Timesheet</h3>
              <button
            ...
            <form onSubmit={handleReject} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-[10px] font-black text-zinc-450 uppercase tracking-widest mb-2">Rejection Reason</label>
                <textarea
                  value={rejectReason}
                  ...
                  className="w-full bg-[#1b1c25] border border-zinc-800 rounded-lg px-3 py-2 text-xs text-zinc-300 focus:outline-none focus:border-violet-600 placeholder-zinc-550 font-semibold"
                />
              </div>
              ...
              <div className="pt-2 border-t border-zinc-850 flex gap-2">
                ...
                <button
                  ...
                  className="flex-1 py-2.5 bg-rose-600 hover:bg-rose-700 text-white rounded-lg font-bold text-xs uppercase tracking-wider flex items-center justify-center transition-colors disabled:opacity-50"
                >
                  {rejectTimesheetMutation.isPending ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <XCircle className="h-4 w-4 mr-2" />}
                  Confirm Reject
                </button>
              </div>
            </form>
