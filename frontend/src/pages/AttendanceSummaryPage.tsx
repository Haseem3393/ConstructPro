      ...
      <div className="space-y-6">
        ...
        <div className="bg-[#0d1322]/70 border border-white/10 rounded-xl p-6 shadow-xl space-y-4 backdrop-blur-xl relative">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-2xl">
            <div>
              <label className="block text-[10px] font-black text-slate-450 uppercase tracking-widest mb-2">Commencement Date</label>
              <input
                type="date"
                value={startDate}
                onChange={(e) => setStartDate(e.target.value)}
                className="w-full bg-[#0a0f1d]/60 border border-white/10 rounded-xl px-3 py-2.5 text-xs text-slate-200 focus:outline-none focus:border-[#7c3aed] hover:border-white/20 transition-all font-semibold"
              />
            </div>

            <div>
              <label className="block text-[10px] font-black text-slate-450 uppercase tracking-widest mb-2">End Date</label>
              <input
                type="date"
                value={endDate}
        ...
        {isLoading ? (
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-[#0d1322]/70 border border-white/10 backdrop-blur-xl rounded-xl p-5 space-y-2">
                <span className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center">
                  <Users className="h-3.5 w-3.5 mr-1 text-[#7c3aed]" /> Active Logged Labor
                </span>
                <span className="block font-black text-white text-xl">{totalWorkersCount} workers</span>
              </div>
              <div className="bg-[#0d1322]/70 border border-white/10 backdrop-blur-xl rounded-xl p-5 space-y-2">
                <span className="block text-[10px] font-bold text-[#00d2ff] uppercase tracking-wider flex items-center">
                  <ClipboardList className="h-3.5 w-3.5 mr-1 text-[#00d2ff]" /> Total Man-Days Logged
                </span>
                <span className="block font-black text-[#00d2ff] text-xl">{totalManDays} days</span>
              </div>
              <div className="bg-[#0d1322]/70 border border-white/10 backdrop-blur-xl rounded-xl p-5 space-y-2">
                <span className="block text-[10px] font-bold text-emerald-450 uppercase tracking-wider flex items-center">
                  <DollarSign className="h-3.5 w-3.5 mr-1 text-emerald-500" /> Cumulative Wages Liability
                </span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              {/* SVG Chart card */}
              <div className="lg:col-span-2 bg-[#0d1322]/70 border border-white/10 rounded-xl p-6 space-y-4 flex flex-col backdrop-blur-xl relative">
                <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-[#7c3aed] via-[#00d2ff] to-transparent" />
                <h3 className="font-bold text-sm text-slate-350 flex items-center uppercase tracking-widest text-[10px]">
                  <BarChart2 className="h-4.5 w-4.5 mr-1.5 text-slate-500" /> Project Attendance Rate (%)
                </h3>

                <div className="flex-1 w-full bg-[#0a0f1d]/60 border border-white/10 rounded-xl p-3 flex items-center justify-center min-h-[240px]">
              ...
              <div className="bg-[#0d1322]/70 border border-white/10 rounded-xl overflow-hidden shadow-xl flex flex-col backdrop-blur-xl">
                <div className="px-6 py-4 border-b border-white/10 bg-white/[0.01]">
                  <h3 className="font-bold text-xs text-slate-355 uppercase tracking-widest">Labour Liabilities</h3>
                </div>
                <div className="flex-1 divide-y divide-white/10 overflow-y-auto max-h-[250px]">
                  ...
                  {projectCosts.length === 0 && (
                    <div className="p-8 text-center text-slate-500 text-xs font-semibold">No records available.</div>
                  )}
                </div>
              </div>
            </div>
