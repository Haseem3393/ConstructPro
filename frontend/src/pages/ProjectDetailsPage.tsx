    ...
    return (
      <SidebarLayout>
        <div className="flex flex-col items-center justify-center py-40 space-y-4">
          ...
          <p className="text-slate-400 font-medium text-sm">Gathering site logs...</p>
        </div>
      </SidebarLayout>
    )
  }
  ...
  return (
    <SidebarLayout>
      <div className="space-y-6">
        {/* Header navigation */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#1a2535] pb-5">
            ...
            <div>
              <div className="flex items-center space-x-3">
                ...
                <span className={`inline-block px-2.5 py-0.5 text-[9px] font-black rounded-full uppercase ${
                  project.status === 'ONGOING' ? 'bg-blue-500/10 text-blue-400 border border-blue-500/22' :
                  project.status === 'COMPLETED' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/22' :
                  project.status === 'OVERDUE' ? 'bg-rose-500/10 text-rose-450 border border-rose-500/22' :
        ...
        {/* Tab Selection */}
        <div className="flex space-x-1 bg-[#0d1526] border border-[#1a2535] p-1 rounded-xl w-fit">
          {(['overview', 'milestones', 'workforce', 'expenses', 'payments'] as const).map(tab => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all duration-155 ${
                activeTab === tab 
                  ? 'bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow shadow-blue-500/10' 
        ...
        {activeTab === 'overview' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-[#0d1526] border border-[#1a2535] rounded-2xl p-6 space-y-5 shadow-lg relative overflow-hidden">
                ...
                {isBudgetWarning && (
                  <div className="p-3.5 bg-rose-500/8 border border-rose-500/20 text-rose-400 text-xs rounded-xl flex items-start space-x-2 font-semibold">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="space-y-1">
                    <span className="block text-[10px] font-bold text-slate-600 uppercase">Capital Budget</span>
                    <span className="block font-black text-white text-base">{formatCurrency(project.budget)}</span>
                  </div>
                  <div className="space-y-1">
                    <span className="block text-[10px] font-bold text-slate-600 uppercase">Logged Expenses</span>
                    <span className="block font-black text-white text-base">{formatCurrency(totalSpent)}</span>
                  </div>
                  <div className="space-y-1">
                    <span className="block text-[10px] font-bold text-slate-600 uppercase">Balance Funds</span>
                    <span className="block font-black text-white text-base">{formatCurrency(project.budget - totalSpent)}</span>
                  </div>
                </div>

                <div className="space-y-2 pt-2 border-t border-[#1a2535]">
                  ...
                  <div className="w-full bg-[#0b1220] rounded-full h-1.5 overflow-hidden border border-white/[0.02]">
                    <div 
                      className={`h-full rounded-full transition-all ${
                        isBudgetWarning ? 'bg-rose-500' : 'bg-blue-500'
                      }`}
              ...
              <div className="bg-[#0d1526] border border-[#1a2535] rounded-2xl p-6 space-y-4 shadow-lg relative overflow-hidden">
                <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-blue-500 to-transparent" />
                <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest">Site Progress Timeline</h3>
                ...
                <div className="grid grid-cols-2 gap-4 text-xs font-bold text-slate-500 pt-3 border-t border-[#1a2535]">
                  <div className="flex items-center">
                    <Calendar className="h-4 w-4 mr-2 text-slate-700 shrink-0" />
                    <span>Commenced: {new Date(project.startDate).toLocaleDateString()}</span>
                  </div>
                  <div className="flex items-center">
                    <Clock className="h-4 w-4 mr-2 text-slate-700 shrink-0" />
            ...
            <div className="space-y-6">
              <div className="bg-[#0d1526] border border-[#1a2535] rounded-2xl p-6 space-y-4 shadow-lg relative overflow-hidden">
                <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-blue-500 to-transparent" />
                <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest">Key Personnel</h3>
                
                <div className="space-y-3.5">
                  <div className="flex items-start space-x-3">
                    <div className="w-9 h-9 bg-blue-500/10 border border-blue-500/22 rounded-xl flex items-center justify-center font-black text-blue-400 shrink-0 text-xs">
                      PM
                    </div>
                    <div>
                      <span className="block text-[10px] font-bold text-slate-600 uppercase tracking-wider">Project Manager</span>
                  ...
                  <div className="flex items-start space-x-3 pt-3.5 border-t border-[#1a2535]">
                    ...
                    <div>
                      <span className="block text-[10px] font-bold text-slate-600 uppercase tracking-wider">Building Owner / Client</span>
              ...
              {project.description && (
                <div className="bg-[#0d1526] border border-[#1a2535] rounded-2xl p-6 space-y-2 shadow-lg relative overflow-hidden">
                  <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-blue-500 to-transparent" />
                  <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Scope of Works</h3>
        ...
        {activeTab === 'milestones' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 bg-[#0d1526] border border-[#1a2535] rounded-2xl p-6 space-y-4 shadow-lg relative overflow-hidden">
              <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-blue-500 to-transparent" />
              ...
              <div className="space-y-3 pt-2">
                {project.milestones?.map((milestone: any) => (
                  <div 
                    key={milestone.id} 
                    className="flex justify-between items-center bg-[#0b1220] border border-[#1a2535] p-4 rounded-xl hover:border-[#253550] transition-colors"
                  >
                    <div className="flex items-start space-x-3">
                      <div className={`p-1.5 rounded-lg mt-0.5 shrink-0 ${
                        milestone.completed 
                          ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/22' 
                          : 'bg-white/[0.03] text-slate-600 border border-[#1a2535]'
                      }`}>
            ...
            <div className="bg-[#0d1526] border border-[#1a2535] rounded-2xl p-6 flex flex-col justify-between shadow-lg relative overflow-hidden">
              <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-blue-500 to-transparent" />
              <h3 className="font-black text-sm text-white border-b border-[#1a2535] pb-3 mb-4">Add Site Milestone</h3>
              ...
              <form onSubmit={handleAddMilestone} className="space-y-4 flex-1">
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-600 uppercase tracking-[0.14em] mb-1.5">Milestone Name *</label>
                  <input
                    type="text"
                    required
                    value={mName}
                    ...
                    className="w-full bg-[#0b1220] border border-[#1a2535] hover:border-[#253550] rounded-xl px-4 py-2.5 text-xs text-slate-200 placeholder-slate-800 focus:outline-none focus:border-blue-500/60 focus:bg-white/[0.05] transition-all duration-200"
                  />
                </div>

                <div>
                  <label className="block text-[10px] font-black text-slate-600 uppercase tracking-[0.14em] mb-1.5">Est. Completion Date *</label>
                  <input
                    type="date"
                    required
                    value={mDueDate}
                    onChange={(e) => setMDueDate(e.target.value)}
                    className="w-full bg-[#0b1220] border border-[#1a2535] hover:border-[#253550] rounded-xl px-4 py-2.5 text-xs text-slate-200 focus:outline-none focus:border-blue-500/60 focus:bg-white/[0.05] transition-all duration-200"
                  />
                </div>

                <div>
                  <label className="block text-[10px] font-black text-slate-600 uppercase tracking-[0.14em] mb-1.5">Scope description</label>
                  <textarea
                    value={mDesc}
        ...
        {activeTab === 'workforce' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 bg-[#0d1526] border border-[#1a2535] rounded-2xl overflow-hidden shadow-lg">
              <div className="px-6 py-4 border-b border-[#1a2535] bg-white/[0.005] flex justify-between items-center">
                ...
                <span className="text-[9px] bg-blue-500/10 text-blue-400 border border-blue-500/22 px-2.5 py-0.5 rounded-full font-black uppercase">
                    ...
                    {project.members?.map((member: any) => (
                      <tr key={member.id} className="hover:bg-white/[0.015] transition-colors">
                        ...
                        <td className="py-4 px-4">
                          <span className="inline-block px-2.5 py-0.5 text-[9px] font-extrabold rounded bg-[#0b1220] border border-[#1a2535] text-slate-400 uppercase">
                            {member.user?.role}
                          </span>
                        </td>
            ...
            <div className="bg-[#0d1526] border border-[#1a2535] rounded-2xl p-6 flex flex-col justify-between shadow-lg relative overflow-hidden">
              <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-blue-500 to-transparent" />
              <h3 className="font-black text-sm text-white border-b border-[#1a2535] pb-3 mb-4">Assign Staff member</h3>
              ...
              <form onSubmit={handleAddMember} className="space-y-4 flex-1">
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-600 uppercase tracking-[0.14em] mb-1.5">Select User *</label>
                  <select
                    value={memUserId}
                    onChange={(e) => setMemUserId(e.target.value)}
                    className="w-full bg-[#0b1220] border border-[#1a2535] hover:border-[#253550] rounded-xl px-3 py-2.5 text-xs text-slate-350 focus:outline-none focus:border-blue-500/60 focus:bg-white/[0.05] transition-all duration-200"
                  >
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-600 uppercase tracking-[0.14em] mb-1.5">Project Role (Scaffolding/Foreman/Engineer)</label>
                  <input
                    type="text"
                    value={memRole}
        ...
        {activeTab === 'expenses' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      ...
                      <tr key={expense.id} className="hover:bg-white/[0.015] transition-colors">
                        ...
                        <td className="py-4 px-4">
                          <span className={`inline-block px-2.5 py-0.5 text-[9px] font-extrabold rounded uppercase border ${
                            expense.category === 'LABOUR' ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/22' :
                            expense.category === 'MATERIAL' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/22' :
                            expense.category === 'EQUIPMENT' ? 'bg-blue-500/10 text-blue-400 border-blue-500/22' :
                            'bg-slate-500/10 text-slate-400 border-slate-500/22'
                          }`}>
                            {expense.category}
                          </span>
                        </td>
            ...
            <div className="bg-[#0d1526] border border-[#1a2535] rounded-2xl p-6 flex flex-col justify-between shadow-lg relative overflow-hidden">
              <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-blue-500 to-transparent" />
              <h3 className="font-black text-sm text-white border-b border-[#1a2535] pb-3 mb-4">Record Site Expense</h3>
              ...
              <form onSubmit={handleAddExpense} className="space-y-4 flex-1">
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-600 uppercase tracking-[0.14em] mb-1.5">Settled Amount (LKR) *</label>
                  <input
                    type="number"
                    required
                    value={expAmount}
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-600 uppercase tracking-[0.14em] mb-1.5">Billing Category *</label>
                  <select
                    value={expCategory}
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-600 uppercase tracking-[0.14em] mb-1.5">Log Date *</label>
                  <input
                    type="date"
                    required
                    value={expDate}
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-600 uppercase tracking-[0.14em] mb-1.5">Auditing description</label>
                  <textarea
                    value={expDesc}
        ...
        {activeTab === 'payments' && (
          <div className="space-y-6 animate-in fade-in duration-200">
            {isPaymentsLoading ? (
              ...
              <div className="bg-[#0d1526] border border-[#1a2535] rounded-2xl p-8 text-center shadow-lg relative overflow-hidden">
                <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-blue-500 to-transparent" />
              ...
              return (
                <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="bg-[#0d1526] border border-[#1a2535] rounded-2xl p-5 space-y-2 relative overflow-hidden shadow-lg">
                      <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-slate-500 to-transparent" />
                      <span className="block text-[10px] font-black text-slate-600 uppercase tracking-wider">Contract Value</span>
                  ...
                  <div className="bg-[#0d1526] border border-[#1a2535] rounded-2xl p-6 space-y-3 shadow-lg relative overflow-hidden">
                    <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-blue-500 to-transparent" />
                  ...
                  <div className="bg-[#0d1526] border border-[#1a2535] rounded-2xl overflow-hidden shadow-xl">
                    <div className="px-6 py-4 border-b border-[#1a2535] bg-white/[0.005] flex justify-between items-center">
                      ...
                      {user?.role === 'CLIENT' && (
                        <span className="text-[9px] bg-blue-500/10 text-blue-400 border border-blue-500/22 px-2.5 py-0.5 rounded-full font-black uppercase tracking-wider">
                              ...
                              <tr key={payment.id} className="hover:bg-white/[0.015] transition-colors">
                                ...
                                <td className="py-4 px-4">
                                  <span className={`inline-flex items-center px-2.5 py-0.5 text-[9px] font-black rounded-full uppercase border ${
                                    ...
                                    'bg-slate-500/10 text-slate-450 border-slate-550/22'
                                  }`}>
