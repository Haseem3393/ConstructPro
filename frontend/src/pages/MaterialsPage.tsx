...
const MaterialsPage: React.FC = () => {
  ...
  const getStatusBadge = (current: number, min: number) => {
    if (current === 0) {
      return (
        <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider bg-rose-500/10 text-rose-455 border border-rose-500/25">
          Out of Stock
        </span>
      )
    }
    if (current <= min) {
      return (
        <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider bg-amber-500/10 text-amber-400 border border-amber-500/25">
          Low Stock
        </span>
      )
    }
    return (
      <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider bg-green-500/10 text-green-400 border border-green-500/25">
        Good Stock
      </span>
    )
  }
    ...
    <SidebarLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-zinc-800 pb-5">
          <div>
            <h1 className="text-3xl font-extrabold text-white">Inventory Materials</h1>
            <p className="text-zinc-400 text-sm mt-1">Track warehouse stock indices, deliveries, and consumptions per project</p>
          </div>
        ...
        {lowStockAlerts.length > 0 && (
          <div className="bg-rose-500/10 border border-rose-500/25 rounded-xl p-4 flex items-start space-x-4 animate-pulse">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {lowStockAlerts.map(m => (
                  <span key={m.id} className="text-[10px] font-bold px-2 py-0.5 bg-rose-500/20 text-rose-400 border border-rose-500/30 rounded">
        ...
        <div className="bg-[#14161f] border border-zinc-800 rounded-xl p-6 shadow-xl space-y-4">
          <h3 className="text-zinc-450 text-xs font-bold uppercase tracking-widest flex items-center">
            <Filter className="h-4 w-4 mr-1.5 text-zinc-550" /> Filter Inventory
          </h3>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-[#1c1d26] border border-zinc-800 rounded-lg pl-9 pr-3 py-1.5 text-xs text-zinc-300 focus:outline-none focus:border-violet-600"
                />
              </div>
            </div>
          </div>
        ...
        <div className="bg-[#14161f] border border-zinc-800 rounded-xl overflow-hidden shadow-xl">
          <div className="px-6 py-4 border-b border-zinc-800 bg-[#171924]/30 flex justify-between items-center">
            <h3 className="font-bold text-sm text-zinc-350">Material Registry</h3>
          </div>

          {isMaterialsLoading ? (
            <div className="flex flex-col items-center justify-center py-20 space-y-3">
              <Loader2 className="h-8 w-8 text-violet-500 animate-spin" />
              <p className="text-xs text-zinc-400 font-medium">Synchronizing site inventory logs...</p>
            </div>
          ) : materialsError ? (
            <div className="p-16 text-center text-rose-400 font-medium text-sm">
              ...
              <table className="w-full text-left border-collapse min-w-[800px]">
                ...
                <tbody className="divide-y divide-zinc-800/60 text-xs">
                  {materials.map((material) => {
                    ...
                    return (
                      <tr key={material.id} className="hover:bg-[#181a24]/20 transition-colors">
                        ...
                        <td className="py-4 px-4 text-zinc-300 font-semibold">{material.project?.name}</td>
                        <td className="py-4 px-4 text-zinc-400">{material.category || 'Other'}</td>
                        ...
                        <td className={`py-4 px-4 text-center font-black ${isLow ? 'text-rose-455' : 'text-green-400'}`}>
                        ...
                        <td className="py-4 px-6">
                          <div className="flex items-center justify-end space-x-2">
                            <Link
                              ...
                              className="p-1.5 bg-zinc-800 hover:bg-zinc-755 text-zinc-350 hover:text-white rounded border border-zinc-700/80"
                              title="View Details"
                            >
                            ...
                            {canTransact && (
                              <button
                                ...
                                className="p-1.5 bg-green-600/10 hover:bg-green-600/20 text-green-400 hover:text-green-300 rounded transition-colors border border-green-500/20 flex items-center"
                            ...
                            {canTransact && (
                              <button
                                ...
                                className="p-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 hover:text-rose-300 rounded transition-colors border border-rose-500/20 flex items-center"
                            ...
                            {isEditable && (
                              <button
                                ...
                                className="p-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-455 rounded border border-rose-500/20"
      ...
      {activeModal === 'stockIn' && selectedMaterial && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-[#14161f] border border-zinc-800 rounded-xl overflow-hidden max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-150">
            <div className="flex justify-between items-center px-6 py-4 border-b border-zinc-800 bg-[#171924]/40">
              <h3 className="font-extrabold text-sm text-white uppercase tracking-wider flex items-center">
                <Truck className="h-4.5 w-4.5 text-green-500 mr-2" />
                Log Delivery (Stock In)
              </h3>
              <button onClick={closeAllModals} className="text-zinc-500 hover:text-white transition-colors">
                <X className="h-4.5 w-4.5" />
              </button>
            </div>
            
            <form onSubmit={handleStockInSubmit} className="p-6 space-y-4">
              <div className="p-3 bg-zinc-850 border border-zinc-800 rounded-lg flex items-center space-x-3 text-xs">
                <Info className="h-4 w-4 text-violet-400 shrink-0" />
                <div>
                  <span className="block text-zinc-400">Logging delivery for: <strong className="text-white">{selectedMaterial.name}</strong></span>
                  <span className="text-[10px] text-zinc-500">Standard unit: {selectedMaterial.unit} • Current stock: {selectedMaterial.currentStock}</span>
                </div>
              </div>

              {modalError && (
                <div className="p-2.5 bg-rose-500/10 border border-rose-500/25 text-rose-455 text-xs rounded font-semibold">
                  {modalError}
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">Quantity Received *</label>
                  <input
                    type="number"
                    required
                    min="0.1"
                    ...
                    className="w-full bg-[#1b1c25] border border-zinc-800 rounded-lg px-3.5 py-2 text-xs text-zinc-250 focus:outline-none focus:border-violet-600 font-semibold"
                  />
                </div>

                <div>
                  <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">Total Cost (LKR)</label>
                  <input
                    type="number"
                    min="0"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">Supplier</label>
                  <select
                    value={txnSupplierId}
                    onChange={(e) => setTxnSupplierId(e.target.value)}
                    className="w-full bg-[#1b1c25] border border-zinc-800 rounded-lg px-3 py-2 text-xs text-zinc-250 focus:outline-none focus:border-violet-600 font-semibold cursor-pointer"
                  >
                ...
                <div>
                  <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">Invoice Number</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">Delivery Date</label>
                  <input
                    type="date"
                    required
                    value={txnDate}
                    onChange={(e) => setTxnDate(e.target.value)}
                    className="w-full bg-[#1b1c25] border border-zinc-800 rounded-lg px-3.5 py-2 text-xs text-zinc-200 focus:outline-none focus:border-violet-600"
                  />
                </div>
              ...
              <div className="flex justify-end space-x-2 pt-4 border-t border-zinc-800/80">
                <button
                  type="button"
                  onClick={closeAllModals}
                  className="px-4 py-2 border border-zinc-800 text-zinc-400 hover:bg-zinc-850 rounded-lg text-xs font-bold uppercase tracking-wider transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-colors disabled:opacity-50"
                >
                  {recordStockInMutation.isPending && <Loader2 className="h-3.5 w-3.5 mr-2 animate-spin" />}
                  Submit Delivery
                </button>
              </div>
            </form>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-[#14161f] border border-zinc-800 rounded-xl overflow-hidden max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleStockOutSubmit} className="p-6 space-y-4">
              <div className="p-3 bg-zinc-850 border border-zinc-800 rounded-lg flex items-center space-x-3 text-xs">
                <Inbox className="h-4 w-4 text-violet-400 shrink-0" />
                <div>
                  <span className="block text-zinc-400">Recording usage of: <strong className="text-white">{selectedMaterial.name}</strong></span>
                  <span className="text-[10px] text-zinc-550">Available stock: <strong className="text-zinc-300">{selectedMaterial.currentStock} {selectedMaterial.unit}</strong></span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">Quantity Consumed *</label>
                  <input
                    type="number"
                    required
                    min="0.1"
                ...
                <div>
                  <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">Date of Consumption</label>
                  <input
                    type="date"
                    required
                    value={txnDate}
              ...
              <div>
                <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">Consumption Usage details</label>
                <textarea
                  required
              ...
              <div className="flex justify-end space-x-2 pt-4 border-t border-zinc-800/80">
                ...
                <button
                  ...
                  className="flex items-center px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-colors disabled:opacity-50"
                >
                  {recordStockOutMutation.isPending && <Loader2 className="h-3.5 w-3.5 mr-2 animate-spin" />}
                  Record Consumption
                </button>
              </div>
            </form>
